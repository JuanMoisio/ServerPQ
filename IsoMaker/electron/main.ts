// electron/main.ts — ESM/TS limpio para tsx
...
async function loadWinPE() {
  ...
  for (const href of tryHrefs) {
    try { const m = await import(href); return unwrap(m); } catch { /* siguiente */ }
  }
  throw new Error("No pude cargar services/winpe.js (compilá TS a JS).");
}
...
async function createWindow() {
  ...
  preload: isDev
    ? require('node:path').resolve(process.cwd(), 'electron', 'preload.js')
    : require('node:path').resolve(__dirname2, 'preload.js'),
},
  });
...
// REPO + HASH + DESCARGA
ipcMain.handle("repo:index", async (_evt, baseUrl: string) => {
...
ipcMain.handle("repo:downloadStart", async (evt, payload) => {
  const wc = evt.sender;
  const r = await loadRepo();
  const sendProg = (data: any) => wc.send("repo:progress", data);
  ...
  try {
    if (payload.driveLetter) {
      ...
      wc.send("repo:done", {
        ok: true, outPath: res.outPath, digest: res.digest,
        match: payload.sha256 ? (res.digest?.toUpperCase() === payload.sha256.toUpperCase()) : null
      });
      return { ok: true, outPath: res.outPath, digest: res.digest };
    } else {
      const out = await r.downloadToWithProgress(payload.url, payload.outDir || "C:\\Temp\\isos", sendProg);
      let vr: any = null;
      if (payload.sha256) vr = await r.verifySha256(out, payload.sha256);
